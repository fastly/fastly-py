...
with fastly.ApiClient(configuration) as api_client:
    ...
    try:
        # Get services with product enabled
        api_response = api_instance.get_services_product_domain_inspector()
        pprint(api_response)
    except fastly.ApiException as e:
        print("Exception when calling ProductDomainInspectorApi->get_services_product_domain_inspector: %s\n" % e)
```
