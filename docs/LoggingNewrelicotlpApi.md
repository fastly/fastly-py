...
with fastly.ApiClient(configuration) as api_client:
    ...
    try:
        # Create a New Relic OTLP endpoint
        api_response = api_instance.create_log_newrelicotlp(service_id, version_id, name=name, placement=placement, response_condition=response_condition, format=format, log_processing_region=log_processing_region, format_version=format_version, token=token, region=region, url=url)
        pprint(api_response)
    except fastly.ApiException as e:
        print("Exception when calling LoggingNewrelicotlpApi->create_log_newrelicotlp: %s\n" % e)
```
...
with fastly.ApiClient(configuration) as api_client:
    ...
    try:
        # Update a New Relic log endpoint
        api_response = api_instance.update_log_newrelicotlp(service_id, version_id, logging_newrelicotlp_name, name=name, placement=placement, response_condition=response_condition, format=format, log_processing_region=log_processing_region, format_version=format_version, token=token, region=region, url=url)
        pprint(api_response)
    except fastly.ApiException as e:
        print("Exception when calling LoggingNewrelicotlpApi->update_log_newrelicotlp: %s\n" % e)
```
